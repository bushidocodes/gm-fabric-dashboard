...
describe("GMLineChart utils functions", () => {
  ...
  test("screenReaderGraphDescription provides correct output ", () => {
    ...
    // indent matters for the string below
    const mockOutputIfData = `A tabular representation of the Chart chart data:
      median 2.00
      average 2.00
      ...
      complete data time series follows 1,2,3`;
    ...
    expect(screenReaderGraphDescription(mockTimeSeriesArray, "Chart")).toEqual(
      mockOutputIfData
    );
    expect(
      screenReaderGraphDescription(mockTimeSeriesArrayNoData, "Chart")
